...
PolyhedronArrayDisplay::PolyhedronArrayDisplay() {
 ...
 mesh_color_property_ =
      new rviz::ColorProperty("MeshColor", QColor(rand()%(256), rand()%(256), rand()%(256)), "Mesh color.",
                              this, SLOT(updateMeshColorAndAlpha()));
  bound_color_property_ =
      new rviz::ColorProperty("BoundColor", QColor(rand()%(256), rand()%(256), rand()%(256)), "Bound color.",
                              this, SLOT(updateBoundColorAndAlpha()));
  ...
  vs_color_property_ =
    new rviz::ColorProperty("VsColor", QColor(rand()%(256), rand()%(256), rand()%(256)), "Vs color.",
                            this, SLOT(updateVsColorAndAlpha()));
...
// poly_info.color.setHSB(1.0f,1.0f,1.0f);
}
...
void PolyhedronArrayDisplay::processMessage(const decomp_ros_msgs::PolyhedronArray::ConstPtr &msg) {
  ...
  for(const auto& polyhedron: polys){
    ...
    // float hue=std::min(1.0,(count/polys.size()));
    // poly_info.color.setHSB(hue,1.0f,1.0f);
    bds.clear();
    bds = cal_vertices(polyhedron);
    ...
    // visualizeMessage(state,poly_info);
    // visualizeMessage(state,DecompROS::polyhedron_to_ros(polyhedron));
    // i++;
  }
...
void PolyhedronArrayDisplay::visualizeMesh() {
  ...
  visual_mesh->setColor(rand()%(256),rand()%(256), rand()%(256), alpha);
  visual_mesh_ = visual_mesh;
// for(size_t i=0;i<poly_infos_.size();++i)
// { 
  // const PolyInfo poly_info=poly_infos_[i];
  // visual_mesh->setColor(poly_info.color.r, poly_info.color.g, poly_info.color.b, alpha);
  // visual_mesh_ = visual_mesh;
}
void PolyhedronArrayDisplay::visualizeMesh(const decomp_ros_msgs::Polyhedron rostomsg_) {
    if (!context_->getFrameManager()->getTransform(
          rostomsg_.header.frame_id, rostomsg_.header.stamp, position_one, orientation_one)) {
    ROS_DEBUG("Error transforming from frame '%s' to frame '%s'",
              rostomsg_.header.frame_id.c_str(), qPrintable(fixed_frame_));
    return;
  }
  std::shared_ptr<MeshVisual> visual_mesh;
  visual_mesh.reset(new MeshVisual(context_->getSceneManager(), scene_node_));

  visual_mesh->setMessage(bds);
  visual_mesh->setFramePosition(position_one);
  visual_mesh->setFrameOrientation(orientation_one);
...
void PolyhedronArrayDisplay::visualizeBound() {
  ...
  visual_bound->setColor(rand()%(256), rand()%(256), rand()%(256), 1.0);
  float scale = scale_property_->getFloat();
  visual_bound->setScale(scale);

  visual_bound_ = visual_bound;
}
void PolyhedronArrayDisplay::visualizeBound(const decomp_ros_msgs::Polyhedron rostomsg_) {
  if (!context_->getFrameManager()->getTransform(
          rostomsg_.header.frame_id, rostomsg_.header.stamp, position_one, orientation_one)) {
    ROS_DEBUG("Error transforming from frame '%s' to frame '%s'",
              rostomsg_.header.frame_id.c_str(), qPrintable(fixed_frame_));
  ...
  visual_bound->setMessage(bds);
  visual_bound->setFramePosition(position_one);
  visual_bound->setFrameOrientation(orientation_one);
...
void PolyhedronArrayDisplay::visualizeMessage(int state,const decomp_ros_msgs::Polyhedron rostomsg_) {
  switch (state) {
  case 0:
    visual_bound_ = nullptr;
    visual_vector_ = nullptr;
