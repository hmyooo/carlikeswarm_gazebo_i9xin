...
private:
  ...
  void visualizeMessage(int state,const decomp_ros_msgs::Polyhedron rostomsg_);
  void visualizeMessage(int state);

  // void visualizeMessage();

  void visualizeMesh();
  void visualizeMesh(const decomp_ros_msgs::Polyhedron rostomsg_);
  void visualizeBound(const decomp_ros_msgs::Polyhedron rostomsg_);
  ...
  // std::vector<PolyInfo> poly_infos_;
  Ogre::Vector3 position_;
  Ogre::Vector3 position_one;

  Ogre::Quaternion orientation_;
  Ogre::Quaternion orientation_one;
