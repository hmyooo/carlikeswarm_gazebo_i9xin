...
namespace plan_manage
{
  ...
  private:
    ...
    bool ifdynamic_;
    int traj_seg_num_;
    MappingProcess::Ptr foropt_map_ptr_;
    ...
    double wei_obs_,wei_obs_2;                         // obstacle weight
    ...
    double  max_cur_;       // dynamic limits
    ...
    std::vector<Eigen::Vector2d> vec_le_, vec_lo_,dy_lo_;
  ...
  public:
    ...
    // std::vector<pair<double,int>> t_ids_;
    vector <Eigen::MatrixXd> local_end_corrid;
    std::vector <int> not_in_car_ids;

    double piece_word_time;
 ...
 double dynamicObsGradCostP_alone( const double &omg_j,
                                const int &time_int_pena,
                                const double &t,               // current absolute time
                                const Eigen::Matrix<double, 6, 1> &beta0,
                                const Eigen::Matrix<double, 6, 1> &beta1,
    ...
    void positiveSmoothedRange(const double &x, const double &pos_min, const double &pos_max, double &f, double &df);
  public:
