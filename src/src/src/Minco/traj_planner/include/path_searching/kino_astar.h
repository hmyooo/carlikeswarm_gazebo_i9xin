...
public:
  ...
  private:
    ...
    std::vector<std::vector<PathNodePtr>> path_nodes_s;

    std::vector<Eigen::Vector2d> last_path_pos_, last_path_pos_temp_;
    std::vector<Eigen::Vector3d> vi;
    /*--------------speed planning-----------------*/
    ...
    vector<int>ids_localstart;

    /*------------------------------------------*/
    std::unique_ptr<path_searching::corrid_safe> corrid_safe_Ptr_;
    ...
    /*-------------------------------------------------*/
    nav_msgs::Path the_globalPath;
    std::vector<Eigen::MatrixXd> safe_hPolys_;   
    MappingProcess::Ptr safe_map_ptr_;
    /* ---------- record data ---------- */
    ...
    double prio1_corrid_wei,prio1_global_wei,prio2_global_wei,prio2_corrid_wei,prio2_time1_wei,prio2_time2_wei,smallervec_;
    ...
    void get_RectangleConst(std::vector<Eigen::Vector3d> statelist);
    double non_siguav;
    ompl::base::StateSpacePtr shotptr;
    std::vector<Eigen::MatrixXd>  the_global_corrid;
    std::vector<double>  the_ts_corrid;
  ...
  public:
    ...
    std::vector<Eigen::Vector2d> car_vertex_small_, car_vertex_, car_vertex_big_;//车顶点车框
    double CalculateAcc(double& t);
    double CalculateVec(double& t);
    void getcheck_corrid_id(Eigen::Vector2d target_pos,Eigen::Vector2d start_pos,double &piece_global_start_time);
    enum { REACH_HORIZON = 1, REACH_END = 2,  NO_PATH = 3, REACH_END_BUT_SHOT_FAILS = 4};
    std::vector<std::vector<Eigen::Vector3d>> otherstatelists;
    // std::vector<pair<double,int>> t_ids;
    std::vector<std::vector<double>> vec_s;
    // std::vector<int> id_s;
    void setglobalcorrid(std::vector<Eigen::MatrixXd>  global_corrid,std::vector<double>  ts_corrid);
    /* main API */
    void setParam(ros::NodeHandle& nh);
    // void init(planning::minco::Config cfg_, ros::NodeHandle nh);
    void init(ros::NodeHandle& nh);
    Eigen::Vector2d last_start_pos;
    double max_vel_ = 6.0;
    double max_acc_ = 3.0;
    double serch_one_cost_forcorrid;
    std::vector<Eigen::Vector4d> localtarget_s;
    ...
    bool searchTime(plan_utils::KinoTrajData &flat_trajs, double &start_world_time,double &piece_global_start_time,double &duration_the);
    void getTruncatedposLists();
    void getTruncatedposLists(int id);

    void getSingulNodes();
    Eigen::Vector3d CalculateInitPos(double& t, int& singul);
    int lock_couunt;
    vector<int>ids_localend;
    int localtarget_findnum,record_localtarget_findnum;
    std::vector<int> not_in_car_id;

    // inital semantic map
    // void intialMap(map_utils::TrajPlannerMapItf *map_itf);
    void A_setMap(MappingProcess::Ptr& ptr);
    ...
    void check_dy_obs_CollisionUsingPosAndYaw(const Eigen::Vector3d &state, bool& res);
    void checkCollisionUsingPosAndYawfordyobs(const Eigen::Vector3d &state, bool& res,double &time,Eigen::Vector2d &orig);
    void checkCollisionUsingLine(const Eigen::Vector2d &start_pt, const Eigen::Vector2d &end_pt, bool &res);    
    void checkCollisionUsingLine_tht(const Eigen::Vector2d &start_pt, const Eigen::Vector2d &end_pt, bool &res);    

    void setglobalpath(const nav_msgs::Path thepath);
    Eigen::Vector3d findNearestGlobalpos(const Eigen::Vector2d& start_pos, bool first_search);
    Eigen::Vector4d findlocaltarget(const Eigen::Vector2d& start_pos,Eigen::Vector4d &end_state,bool ifrepeat,double &piece_global_start_time);
    void read_trac_yaml();
    void findlocalpath(Eigen::Vector2d start_pos,int globalplay_id);
    ...
    double checkl = 0.2;
    int last_nearest_idx;
    double last_nearest_idxkk;

    double world_clean_time;
    ros::Publisher expandNodesVis;
    bool lated_lock;
    double record_max_vel_,record_max_acc_;
