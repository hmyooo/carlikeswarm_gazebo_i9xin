...
namespace plan_utils
{
    ...
    public:
                        ...
                        const int &pieceNum)
        {
            ...
            tailPVA = tailState;
            T1.resize(N);
//每个“片段”或多项式段有6个系数（这对于一个五次多项式来说是标准的，它有六个系数：一个常数项，一个一次项，等等，直到一个五次项）。因为有N个这样的段，所以有6 * N行。
//2是列数，这可能表示每个点或每个多项式都有两个关键值需要计算。这可能是因为问题是在二维空间中处理的
            A.create(6 * N, 6, 6);
            b.resize(6 * N, 2);
            gdC.resize(6 * N, 2);
            return;
...
//计算基于时间的多项式。T1 到 T5 是基于时间 ts 的向量，代表不同的时间幂。例如，T2 是时间的平方，T3 是时间的立方，依此类推。
...
//这些系数基于多项式的性质和时间的幂。它们通常是预定的，基于多项式轨迹规划的数学。
//通过 N - 1 的值迭代，其中 N 看起来是轨迹点的数量，这个循环是在为每个轨迹点之间的多项式插值设置方程
            for (int i = 0; i < N - 1; i++)
            {
                A(6 * i + 3, 6 * i + 3) = 6.0;
...
//在循环之后，代码设置了最终点的条件，这是通过添加更多的系数到 A 矩阵和 b 向量来实现的
